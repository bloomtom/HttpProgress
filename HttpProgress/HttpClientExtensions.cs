...
namespace HttpProgress
{
    ...
    public static class HttpClientExtensions
    {
        ...
        public static async Task<HttpResponseMessage> GetAsync(this HttpClient client, string requestUri, Stream destination, IProgress<ICopyProgress> progress = null, CancellationToken cancelToken = default(CancellationToken))
        {
        ...
        public static async Task<HttpResponseMessage> PutAsync(this HttpClient client, string requestUri, Stream content, long expectedContentLength = 0, IProgress<ICopyProgress> progress = null, CancellationToken cancelToken = default(CancellationToken))
        {
            return await client.PutAsync(requestUri, new ProgressStreamContent(content, progress), cancelToken);
        }
        ...
        /// <param name="cancelToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <returns>The full HTTP response.</returns>
        public static async Task<HttpResponseMessage> PostAsync(this HttpClient client, string requestUri, Stream content, long expectedContentLength = 0, IProgress<ICopyProgress> progress = null, CancellationToken cancelToken = default(CancellationToken))
        {
            return await client.PostAsync(requestUri, new ProgressStreamContent(content, progress), cancelToken);
        }
    }
}
